...
async function main() {
  ...
  // Keyed by MEETING KEY, not country: 2026 has three countries hosting
  // more than one round (Bahrain, Spain, United States), so a country map
  // silently gave every one of them a single shared icon.
  const byMeeting = new Map<number, { url: string; country: string }>()
  for (const m of meetingsRaw) {
    if (m.circuit_image) byMeeting.set(m.meeting_key, { url: m.circuit_image, country: m.country_name })
  }
  const circuitEntries = [...byMeeting.entries()].map(([key, { url }]) => ({
    key: String(key),
    url,
    file: `meeting-${key}.${extOf(url)}`,
  }))
  const circuits = await fetchCategory('circuits', 'circuits', circuitEntries)

  // Legacy country map kept so any caller that still has only a country
  // name keeps working; first meeting per country wins, as before.
  const circuitByCountry: Record<string, string> = {}
  for (const m of meetingsRaw) {
    const path = circuits.paths[String(m.meeting_key)]
    if (path && !circuitByCountry[m.country_name]) circuitByCountry[m.country_name] = path
  }
...
export const CIRCUIT_IMAGES_BY_MEETING: Record<string, string> = ${JSON.stringify(circuits.paths, null, 2)}

// legacy country_name map — first meeting per country
export const CIRCUIT_IMAGES: Record<string, string> = ${JSON.stringify(circuitByCountry, null, 2)}
